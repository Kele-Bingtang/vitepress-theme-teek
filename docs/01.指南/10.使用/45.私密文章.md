...
打开 `.vitepress/config.ts` 或 `.vitepress/config.mts` 文件，给 head 模块添加如下信息：
...
比如我们有 `指南` 和 `配置` 两个专题的文章，那么我可以对这两个专题进行认证，在 `.vitepress/teekConfig.ts` 文件配置如下：
...
在 `指南` 的各个文章 `frontmatter` 配置绑定对应的 `realm`：
...
在 `配置` 的各个文章 `frontmatter` 配置绑定对应的 `realm`：
...
const teekConfig = defineTeekConfig({
  private: {
    enabled: true /* ◀️ 启用私密文章功能 */,
    expire: "1d" /* ◀️ 登录过期时间，1d 代表 1 天，1h 代表 1 小时 */,
    session: false /* ◀️ 关闭网页后是否清除登录状态 */,
    /*  全局页面级别登录信息 - 登录一次后可访问所有全局私密文章 */
    pages: [
      { username: "teek-pages-1", password: "teek" },
      { username: "teek-pages-2", password: "teek" },
    ],
  },
});
```
...
在 `.vitepress/teekConfig.ts` 里开启站点级别认证并设置认证信息：
...
const teekConfig = defineTeekConfig({
  private: {
    ...
    session: false /* ◀️ 关闭网页后是否清除登录状态 */,
    siteLogin: true /* ◀️ 开启站点级别登录 */,
    /*  站点级别登录信息，role 为 admin 的账号登录后可访问所有私密文章 */
    site: [
      { username: "teek-site-1", password: "teek" },
      { username: "teek-site-2", password: "teek" },
      { username: "teek-site-3", password: "teek", role: "admin" },
    ],
  },
});
```
